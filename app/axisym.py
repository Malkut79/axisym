...
def disc(thickness = 1, inner_radius=2, outer_radius=10):
    Nx, Ny = 5, 50
...
# left_dofs = fem.locate_dofs_geometrical(V, leftside)
# # print(left_dofs)


# coords = V.tabulate_dof_coordinates()
# len(coords)
# coords = np.array([coords[i] for i in left_dofs])
# sorted_left_dofs = left_dofs[coords[:,1].argsort()]
# sorted_coords = coords[coords[:, 1].argsort()]
# X = [i[1] for i in sorted_coords]
# Y = np.array([ s_dg.x.array[i*9+4] for i in sorted_left_dofs])
# plt.plot(X, Y, '-')